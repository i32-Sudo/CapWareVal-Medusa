...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, window->Size.x - 30, label_size.y + 13);
    ...
    float anim = ImTricks::Animations::FastFloatLerp( std::string( label ).append( "buttonex.anim" ), hovered || held, 0.f, 1.f, 0.04f );
    ...
    RenderText( { bb.GetCenter( ).x - label_size.x / 2, bb.Min.y }, label );
    PopStyleColor( );

    window->DrawList->AddRectFilled( { bb.Min.x, bb.Max.y - 1 }, bb.Max, GetColorU32( ImGuiCol_Border ) );
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x - 15, outer_rect.Max.x - border_size - scrollbar_size - 15), inner_rect.Min.y, outer_rect.Max.x - 15, inner_rect.Max.y - 15);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    RenderFrame( bb_frame.Min, bb_frame.Max, bg_col, true, style.ScrollbarRounding );
    window->DrawList->AddRect( bb_frame.Min, bb_frame.Max, GetColorU32( ImGuiCol_Border ), style.ScrollbarRounding );
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetWindowWidth() - 30;
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 13));
    const ImGuiID popup_id = ImHashStr("##ComboPopup", 0, id);
    bool popup_open = IsPopupOpen(popup_id, ImGuiPopupFlags_None);
    float popup_open_anim = ImTricks::Animations::FastFloatLerp( std::string( label ).append( "begincombo.popup_open_anim" ), popup_open, 0.f, 1.f, 0.04f );
    const ImRect bb(total_bb.Max - ImVec2(24 + CalcTextSize( preview_value ).x + ( 150 - ( 24 + CalcTextSize( preview_value ).x ) ) * popup_open_anim, total_bb.GetHeight( ) + 6), total_bb.Max - ImVec2( 0, 6 ));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    float anim = ImTricks::Animations::FastFloatLerp( std::string( label ).append( "begincombo.anim" ), hovered || popup_open, 0.f, 1.f, 0.04f );
    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), anim );

    window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( ImGuiCol_FrameBg, popup_open_anim ), style.FrameRounding, popup_open ? ImDrawFlags_RoundCornersTop : 15 );

    custom::ImRotateStart( );
    RenderArrow( window->DrawList, { bb.Max.x - 14 - style.FramePadding.x * popup_open_anim, bb.GetCenter( ).y - 17 }, col, ImGuiDir_Right, 20.f );
    custom::ImRotateEnd( 3.14f * popup_open_anim );

    RenderText( { total_bb.Min.x, total_bb.Max.y - label_size.y - 13 }, label );
    PushStyleColor( ImGuiCol_Text, col.Value );
    RenderText( { bb.Min.x + style.FramePadding.x * popup_open_anim, total_bb.Max.y - label_size.y - 13 }, preview_value );
    PopStyleColor( );
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (Selectable(item_text, item_selected, ImGuiSelectableFlags_NoPadWithHalfSpacing, { 0, GetIO( ).Fonts->Fonts[0]->FontSize + 12 }))
        {
            value_changed = true;
            *current_item = i;
        }
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent)
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    ...
    switch (data_type)
    {
    case ImGuiDataType_S8:  { ImS32 v32 = (ImS32)*(ImS8*)p_v;  bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS8*)p_min,  *(const ImS8*)p_max,  format, flags, out_grab_bb, percent); if (r) *(ImS8*)p_v  = (ImS8)v32;  return r; }
    case ImGuiDataType_U8:  { ImU32 v32 = (ImU32)*(ImU8*)p_v;  bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min,  *(const ImU8*)p_max,  format, flags, out_grab_bb, percent); if (r) *(ImU8*)p_v  = (ImU8)v32;  return r; }
    case ImGuiDataType_S16: { ImS32 v32 = (ImS32)*(ImS16*)p_v; bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)p_min, *(const ImS16*)p_max, format, flags, out_grab_bb, percent); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_U16: { ImU32 v32 = (ImU32)*(ImU16*)p_v; bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags, out_grab_bb, percent); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_S32:
        IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
        return SliderBehaviorT<ImS32, ImS32, float >(bb, id, data_type, (ImS32*)p_v,  *(const ImS32*)p_min,  *(const ImS32*)p_max,  format, flags, out_grab_bb, percent);
    case ImGuiDataType_U32:
        IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT<ImU32, ImS32, float >(bb, id, data_type, (ImU32*)p_v,  *(const ImU32*)p_min,  *(const ImU32*)p_max,  format, flags, out_grab_bb, percent);
    case ImGuiDataType_S64:
        IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 && *(const ImS64*)p_max <= IM_S64_MAX / 2);
        return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type, (ImS64*)p_v,  *(const ImS64*)p_min,  *(const ImS64*)p_max,  format, flags, out_grab_bb, percent);
    case ImGuiDataType_U64:
        IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
        return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type, (ImU64*)p_v,  *(const ImU64*)p_min,  *(const ImU64*)p_max,  format, flags, out_grab_bb, percent);
    case ImGuiDataType_Float:
        IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f && *(const float*)p_max <= FLT_MAX / 2.0f);
        return SliderBehaviorT<float, float, float >(bb, id, data_type, (float*)p_v,  *(const float*)p_min,  *(const float*)p_max,  format, flags, out_grab_bb, percent);
    case ImGuiDataType_Double:
        IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f && *(const double*)p_max <= DBL_MAX / 2.0f);
        return SliderBehaviorT<double, double, double>(bb, id, data_type, (double*)p_v, *(const double*)p_min, *(const double*)p_max, format, flags, out_grab_bb, percent);
    case ImGuiDataType_COUNT: break;
    }
    IM_ASSERT(0);
    return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + 12), total_bb.Max - ImVec2( 0, 13 ));
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb, &percent);
    if (value_changed)
        MarkItemEdited(id);
    ...
    value->second = ImLerp( value->second, ( float )percent / 100.f * frame_bb.GetWidth( ), 0.1f );

    RenderText( total_bb.Min, label );
    RenderText( { total_bb.Max.x - CalcTextSize( value_buf ).x, total_bb.Min.y }, value_buf );

    RenderFrame( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_FrameBg ), 1, 2 );
    RenderFrame( frame_bb.Min, { frame_bb.Min.x + value->second, frame_bb.Max.y }, GetColorU32( ImGuiCol_Scheme ), 1, 2 );
    window->DrawList->AddCircleFilled( { frame_bb.Min.x + value->second, frame_bb.GetCenter( ).y }, 8, GetColorU32( ImGuiCol_SliderGrab ) );
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 total_size = CalcItemSize(size_arg, GetWindowWidth( ) - 30, (is_multiline ? g.FontSize * 8.0f : label_size.y + 13));

    float frame_anim = ImTricks::Animations::FastFloatLerp( std::string( label ).append( "inputtextex.frame_anim" ), g.ActiveId == id, 0.f, 1.f, 0.04f );

    const ImVec2 frame_size = ImVec2( style.FramePadding.x * 2 + CalcTextSize( buf ).x + ( CalcTextSize( buf ).x < 50 ? ( 50 - CalcTextSize( buf ).x ) * frame_anim : 0 ), g.FontSize + 13 );
    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), frame_anim );

    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + total_size);
    const ImRect frame_bb({ total_bb.Max.x - frame_size.x, total_bb.Min.y - 6 }, { total_bb.Max.x, total_bb.Min.y - 6 + frame_size.y });
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg, frame_anim), true, style.FrameRounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2( style.FramePadding.x, frame_size.y / 2 - CalcTextSize( buf ).y / 2 );
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + button_offset_x, pos.y - 3);

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags, { 20, 20 }))
        {
        ...
        if (BeginPopup("picker"))
        {
            ...
            ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoInputs;
            SetNextItemWidth(square_sz * 9.0f); // Use 256 + bar sizes?
    ...
    window->DrawList->AddRectFilled( { pos.x, pos.y + 25 }, { pos.x + window->Size.x - 30, pos.y + 26 }, GetColorU32( ImGuiCol_Border ) );
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    float anim = ImTricks::Animations::FastFloatLerp( std::string( label ).append( "selectable.anim" ), hovered || selected, 0.f, selected ? 1.f : 0.5f, 0.04f );
    float anim2 = ImTricks::Animations::FastFloatLerp( std::string( label ).append( "selectable.anim2" ), hovered || selected, 0.f, 1.f, 0.04f );
    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_Text ), anim2 );

    RenderFrame( bb.Min, { bb.Min.x + 1, bb.Max.y }, GetColorU32( ImGuiCol_Scheme, anim ) );
    ...
    PushStyleColor( ImGuiCol_Text, col.Value );
    RenderText( { bb.Min.x + style.FramePadding.x, bb.GetCenter( ).y - label_size.y / 2 }, label );
    PopStyleColor( );
